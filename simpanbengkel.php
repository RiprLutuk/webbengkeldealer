...
//cek validasi (ada atau tidaknya dealer dengan kdbengkel tersebut)
if ($stmt->rowCount() > 0) { //jika sudah ada
    ?>
    <script>
        alert("Data dengan kdbengkel : <?php echo $kdbengkel; ?> SUDAH ADA");
        window.history.back();
    </script>	
    <?php
} else { //jika kdbengkel belum ada
    //proses penyimpanan
    $stmt = $db->prepare('insert into data_bengkel values(:kdbengkel,:nmbengkel,:jmlmontir,'
            . ':haribuka,:jambuka,:alamat,:kelurahan,:kecamatan,:kdpos,:kabupaten,'
            . ':telepon,:latitude,:longitude,:foto,:username,:waktu)');
